...
class SVGLibrary(tk.Frame):
    ...
    Manages the SVG asset library on the left side.

    Responsibilities

    Load SVG files

    Show previews using tksvg

    Search/filter SVGs

    Drag-and-drop SVGs onto the canvas"""
    ...
    def __init__(self, parent, canvas_app):
        ...
        # Header
        header = tk.Label(self, text="Library", font=("Arial", 12, "bold"), bg="#f0f0f0")
        header.pack(pady=5)
    ...
    def end_drag(self, event):
        ...
        if hasattr(self, 'drag_window'):
            ...
            # Adjust for zoom
            canvas_x = self.canvas_app.canvas.canvasx(canvas_x) / self.canvas_app.zoom_manager.zoom_level
            canvas_y = self.canvas_app.canvas.canvasy(canvas_y) / self.canvas_app.zoom_manager.zoom_level
            
            data = self.svg_items[self.drag_item]['data']
            self.canvas_app.svg_manager.add_svg_to_canvas(data, canvas_x, canvas_y)
...
class ZoomManager:
    """Manages zoom operations for the canvas
    Controls zooming behavior of the canvas.
    ...
    def __init__(self, canvas, zoom_label, status_callback):
        self.canvas = canvas
    ...
    def apply_zoom(self):
        ...
        self.canvas.scale("all", 0, 0, self.zoom_level / self._last_zoom, 
                         self.zoom_level / self._last_zoom)
        ...
        self.status_callback(f"Zoom: {int(self.zoom_level * 100)}%")
...
class HistoryManager:
    ...
    Uses a stack-like history list

    Maintains a history_index

    Truncates future history on new action"""

    def __init__(self, canvas, svg_manager, zoom_manager, status_callback):
        self.canvas = canvas
    ...
    def save_state(self):
        """Save full canvas state for undo"""
        state = {
            "items": [],
            "svg_data": self.svg_manager.svg_data.copy(),
            "zoom": self.zoom_manager.zoom_level
        }

        for item in self.canvas.find_withtag("svg"):
            img_name = self._get_image_name(item)
            ...
            state["items"].append({
                ...
                "y": y,
                "tags": tags,
                "width": self.svg_manager.svg_images[img_name].width()
            })

        # Truncate future history if new action is made
    ...
    def undo(self):
        ...
        if self.history_index <= 0:
            self.status_callback("Nothing to undo")
            return
        
        self.history_index -= 1
        self._restore_state(self.history[self.history_index])
        self.status_callback("Undid last action")
    
    def redo(self):
        """Redo the last undone action"""
        if self.history_index >= len(self.history) - 1:
            self.status_callback("Nothing to redo")
            return
        
        self.history_index += 1
        self._restore_state(self.history[self.history_index])
        self.status_callback("Redid last action")
    
    def _restore_state(self, state):
        """Restore canvas from saved state"""
        self.canvas.delete("all")
        self.svg_manager.svg_images.clear()
        self.svg_manager.svg_data = state["svg_data"].copy()
        
        for item_info in state["items"]:
            img_name = item_info["image_name"]
            data = self.svg_manager.svg_data[img_name]
            self.svg_manager.svg_images[img_name] = tksvg.SvgImage(data=data, scaletowidth=item_info["width"])
            
            self.canvas.create_image(item_info["x"], item_info["y"], 
                                   image=self.svg_manager.svg_images[img_name], 
                                   anchor="center", tags=item_info["tags"])
        
        self.zoom_manager.zoom_level = state["zoom"]
        self.zoom_manager.apply_zoom()
    
    def _get_image_name(self, item):
        """Get image name from item tags"""
        return next((t for t in self.canvas.gettags(item) if t.startswith("svg_")), None)


class SVGManager:
    """Manages SVG data and transformations
    Handles SVG data, rendering, and transformations

    Responsibilities

    Store raw SVG XML

    Render SVGs using tksvg

    Apply SVG transforms (rotate, scale, flip)

    Update SVG XML dynamically
    ...
    def __init__(self, canvas, zoom_manager, status_callback, save_state_callback):
        self.canvas = canvas
        self.zoom_manager = zoom_manager
        self.status_callback = status_callback
    ...
    def add_svg_to_canvas(self, data, x, y):
        ...
        try:
            ...
            x *= self.zoom_manager.zoom_level
            y *= self.zoom_manager.zoom_level
            
            self.canvas.create_image(x, y, image=self.svg_images[image_name], 
                                   anchor="center", tags=(image_name, "svg"))
            self.status_callback(f"Added {image_name}")
...
class SelectionManager:
    """Manages item selection and manipulation
        Applies geometric and visual transformations
    ...
    Applies SVG rotate() around center

    Maintains true vector geometry
    ...
    Dialog-based opacity input"""

    def __init__(self, canvas, svg_manager, status_callback, save_state_callback):
        self.canvas = canvas
        self.svg_manager = svg_manager
        self.status_callback = status_callback
        self.save_state_callback = save_state_callback
    ...
    def group_items(self):
        ...
        if len(self.selected_items) > 1:
            ...
            self.status_callback(f"Grouped {len(self.selected_items)} items")
    ...
    def ungroup_items(self):
        ...
        if self.selected_items:
            ...
            if group_id and group_id in self.groups:
                ...
                self.status_callback("Ungrouped items")
                self.draw_selection_box()
        self.save_state_callback()
    ...
    def duplicate_selection(self):
        ...
        for item in self.selected_items:
            ...
            if img_name and img_name in self.svg_manager.svg_data:
                x, y = self.canvas.coords(item)
                data = self.svg_manager.svg_data[img_name]
                
                new_name = f"svg_{len(self.svg_manager.svg_images)}"
                self.svg_manager.svg_data[new_name] = data
                self.svg_manager.svg_images[new_name] = tksvg.SvgImage(data=data, 
                    scaletowidth=self.svg_manager.svg_images[img_name].width())
                
                new_item = self.canvas.create_image(x+20, y+20, 
                    image=self.svg_manager.svg_images[new_name], anchor="center",
                    tags=(new_name, "svg"))
                new_items.append(new_item)
        
        self.selected_items = new_items
        self.draw_selection_box()
        self.status_callback(f"Duplicated {len(new_items)} item(s)")
        self.save_state_callback()
    ...
    def delete_selection(self):
        ...
        for item in self.selected_items:
            ...
            if img_name:
                self.svg_manager.svg_images.pop(img_name, None)
                self.svg_manager.svg_data.pop(img_name, None)
        
        self.selected_items = []
        self.draw_selection_box()
        self.status_callback(f"Deleted {count} item(s)")
    
    def select_all(self):
        """Select all SVG items"""
        self.selected_items = list(self.canvas.find_withtag("svg"))
        self.draw_selection_box()
        self.status_callback(f"Selected all ({len(self.selected_items)} items)")
    ...
    def move_forward(self):
        ...
        self.status_callback("Moved forward")
        self.save_state_callback()
    ...
    def move_backward(self):
        ...
        self.status_callback("Moved backward")
        self.save_state_callback()


class TransformManager:
    """Manages transformations like rotation and flipping"""
    def __init__(self, canvas, svg_manager, selection_manager, status_callback, save_state_callback):
        self.canvas = canvas
        self.svg_manager = svg_manager
    ...
    def rotate_selection(self, angle):
        ...
        if not self.selection_manager.selected_items:
            return
        
        for item in self.selection_manager.selected_items:
            img_name = self.selection_manager.get_image_name(item)
            if img_name:
                self.svg_manager.apply_transform(img_name, f"rotate({angle})", from_center=True)
        
        self.status_callback(f"Rotated {angle}°")
        self.save_state_callback()
    ...
    def flip_selection(self, direction):
        ...
        bbox = self.canvas.bbox(*self.selection_manager.selected_items)
        ...
        for item in self.selection_manager.selected_items:
            ...
            if img_name and img_name in self.svg_manager.svg_images:
                ...
                self.svg_manager.apply_transform(img_name, transform, from_center=True)
        
        self.selection_manager.draw_selection_box()
        self.status_callback(f"Flipped {direction}")
        self.save_state_callback()
    ...
    def adjust_opacity(self):
        ...
        if opacity is not None:
            for item in self.selection_manager.selected_items:
                img_name = self.selection_manager.get_image_name(item)
                if img_name and img_name in self.svg_manager.svg_data:
                    tree = etree.parse(io.BytesIO(self.svg_manager.svg_data[img_name]))
                    root = tree.getroot()
                    root.set('opacity', str(opacity))
                    self.svg_manager.update_svg(img_name, tree)
            
            self.status_callback(f"Set opacity to {opacity}")
        self.save_state_callback()
...
class InteractionHandler:
    ...
    Commit SVG transformation on release"""
    def __init__(self, canvas, svg_manager, selection_manager, zoom_manager, save_state_callback):
        self.canvas = canvas
        self.svg_manager = svg_manager
        self.selection_manager = selection_manager
        self.zoom_manager = zoom_manager
    ...
    def on_press(self, event):
        ...
        if is_handle:
            ...
            group_id = self.selection_manager.get_group_id(item)
            is_shift = event.state & 0x0001
            
            if group_id and item not in self.selection_manager.selected_items:
                self.selection_manager.selected_items = self.selection_manager.groups.get(group_id, [item])[:]
            elif not is_shift:
                if item not in self.selection_manager.selected_items:
                    self.selection_manager.selected_items = [item]
            else:
                if item in self.selection_manager.selected_items:
                    self.selection_manager.selected_items.remove(item)
                else:
                    self.selection_manager.selected_items.append(item)
        else:
            self.selection_manager.selected_items = []
        
        if self.selection_manager.selected_items:
            self.drag_info['start_pos'] = (self.canvas.canvasx(event.x), 
                                          self.canvas.canvasy(event.y))
            self.drag_info['start_bbox'] = self.canvas.bbox(*self.selection_manager.selected_items)
            self.drag_info['item_starts'] = {i: self.canvas.coords(i) for i in self.selection_manager.selected_items}
        
        self.selection_manager.draw_selection_box()
        self.selection_manager.status_callback(f"Selected {len(self.selection_manager.selected_items)} item(s)")
    
    def on_motion(self, event):
        """Handle mouse motion"""
        if not self.drag_info or not self.selection_manager.selected_items:
            return
    ...
    def perform_smooth_resize(self, event_x, event_y):
        """Perform smooth visual resize during drag"""
        if 'resize_initialized' not in self.drag_info:
            self.drag_info['resize_initialized'] = True
            self.drag_info['initial_sizes'] = {}
            for item in self.selection_manager.selected_items:
                img_name = self.selection_manager.get_image_name(item)
                if img_name in self.svg_manager.svg_images:
                    img = self.svg_manager.svg_images[img_name]
        ...
        for item in self.selection_manager.selected_items:
            ...
            img_name = self.selection_manager.get_image_name(item)
            if img_name in self.svg_manager.svg_images and item in self.drag_info['initial_sizes']:
                img = self.svg_manager.svg_images[img_name]
    ...
    def commit_resize(self, event_x, event_y):
        """Commit resize transformation by updating SVG data"""
        handle = self.drag_info['handle']
        x1, y1, x2, y2 = self.drag_info['start_bbox']
        ax = x2 if "left" in handle else x1
        ay = y2 if "top" in handle else y1
        ...
        for item in self.selection_manager.selected_items:
            ...
            if img_name and img_name in self.svg_manager.svg_data:
                if item in self.drag_info.get('initial_sizes', {}):
                    ...
                    self.svg_manager.svg_images[img_name].configure(scaletowidth=new_width)
        self.save_state_callback()


class DrawingCanvas:
    """Main application class coordinating all managers
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("BioIllustrator")
        ...
        # Initialize managers
        self.zoom_manager = ZoomManager(self.canvas, self.zoom_label, self.update_status)
        self.svg_manager = SVGManager(self.canvas, self.zoom_manager, self.update_status, self.save_state)
        self.selection_manager = SelectionManager(self.canvas, self.svg_manager, self.update_status, self.save_state)
        self.history_manager = HistoryManager(self.canvas, self.svg_manager, self.zoom_manager, self.update_status)
        self.transform_manager = TransformManager(self.canvas, self.svg_manager, self.selection_manager, 
                                                  self.update_status, self.save_state)
        self.interaction_handler = InteractionHandler(self.canvas, self.svg_manager, self.selection_manager,
                                                       self.zoom_manager, self.save_state)
    ...
    def bind_events(self):
        ...
        self.canvas.bind("<ButtonPress-1>", self.interaction_handler.on_press)
        self.canvas.bind("<B1-Motion>", self.interaction_handler.on_motion)
        self.canvas.bind("<ButtonRelease-1>", self.interaction_handler.on_release)
    ...
    def import_and_convert_image(self):
        ...
        try:
            ...
            output_path = output_dir / f"converted_{len(self.svg_manager.svg_images)}.svg"
    ...
    def clear_canvas(self):
        ...
        if messagebox.askyesno("Clear Canvas", "Remove all items from canvas?"):
            self.canvas.delete("all")
            self.svg_manager.svg_images.clear()
            self.svg_manager.svg_data.clear()
            self.selection_manager.selected_items.clear()
            self.selection_manager.groups.clear()
            self.zoom_manager.zoom_level = 1.0
            self.zoom_manager._last_zoom = 1.0
            self.zoom_label.config(text="100%")
            self.update_status("Canvas cleared")
    
    # Delegate methods to appropriate managers
    def group_items(self):
        self.selection_manager.group_items()
    
    def ungroup_items(self):
        self.selection_manager.ungroup_items()
    
    def duplicate_selection(self):
        self.selection_manager.duplicate_selection()
    ...
    def nudge_selection(self, dx, dy):
        self.selection_manager.nudge_selection(dx, dy)
    
    def move_forward(self):
        self.selection_manager.move_forward()
    
    def move_backward(self):
        self.selection_manager.move_backward()
    
    def rotate_selection(self, angle):
        self.transform_manager.rotate_selection(angle)
    
    def flip_selection(self, direction):
        self.transform_manager.flip_selection(direction)
    
    def adjust_opacity(self):
        self.transform_manager.adjust_opacity()
