...
class DrawingCanvas:
    ...
    def save_state(self):
        ...
        for item in self.canvas.find_withtag("svg"):
            ...
            state["items"].append({
                ...
                "y": y,
                "tags": tags,
                "width": self.svg_images[img_name].width()
            })
    ...
    def restore_state(self, state):
        ...
        self.svg_data = state["svg_data"].copy()
        self.groups.clear()

        for item in state["items"]:
            img_name = item["image_name"]
            self.svg_images[img_name] = tksvg.SvgImage(
                data=self.svg_data[img_name],
                scaletowidth=item["width"]
            )

            canvas_item = self.canvas.create_image(
                item["x"], item["y"],
                image=self.svg_images[img_name],
                anchor="center",
                tags=item["tags"]
            )
            ...
            for tag in item["tags"]:
                if tag.startswith("group_"):
                    self.groups.setdefault(tag, []).append(canvas_item)
    ...
    def redo(self):
        ...
        for item_info in state["items"]:
            ...
            data = self.svg_data[img_name]
            self.svg_images[img_name] = tksvg.SvgImage(data=data, scaletowidth=item_info["width"])
