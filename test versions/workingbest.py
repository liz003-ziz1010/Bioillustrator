...
class SVGLibrary(tk.Frame):
    ...
    def __init__(self, parent, canvas_app):
        ...
        # Scrollable canvas for library items
        ...
        tk.Button(btn_frame, text="Add SVG", command=self.add_svg_to_library).pack(fill=tk.X, pady=2)
        tk.Button(btn_frame, text="Clear Library", command=self.clear_library).pack(fill=tk.X, pady=2)
    
    def add_svg_to_library(self):
        """Add SVG files to the library"""
        filepaths = filedialog.askopenfilenames(title="Select SVG files", filetypes=[("SVG files", "*.svg")])
        if filepaths:
    ...
    def load_svg_item(self, svg_path):
        ...
        try:
            ...
            item_frame = tk.Frame(self.scrollable_frame, bg="white", relief=tk.RIDGE, bd=1, cursor="hand2")
...
class DrawingCanvas:
    ...
    def setup_layout(self):
        ...
        tk.Button(toolbar, text="⬆", command=self.move_forward, width=3).pack(side=tk.LEFT, padx=2, pady=5)
        tk.Button(toolbar, text="⬇", command=self.move_backward, width=3).pack(side=tk.LEFT, padx=2, pady=5)
        tk.Button(toolbar, text="🗑", command=self.delete_selection, width=3).pack(side=tk.LEFT, padx=2, pady=5)
        tk.Button(toolbar, text="↺", command=lambda: self.rotate_selection(-15), width=3).pack(side=tk.LEFT, padx=2, pady=5)
        tk.Button(toolbar, text="↻", command=lambda: self.rotate_selection(15), width=3).pack(side=tk.LEFT, padx=2, pady=5)
        tk.Button(toolbar, text="📋", command=self.duplicate_selection, width=3).pack(side=tk.LEFT, padx=2, pady=5)
    ...
    def create_ui(self):
        ...
        style_menu.add_command(label="Recolor", command=self.recolor_dialog)
    ...
    def on_motion(self, event):
        ...
        if op_type == 'resize':
            self.perform_visual_resize(canvas_x, canvas_y)
        elif op_type == 'drag':
    ...
    def perform_visual_resize(self, event_x, event_y):
        """Perform visual resize during drag"""
        ...
        for item in self.selected_items:
            ...
            if img_name in self.svg_images:
                ...
                img.configure(scaletowidth=max(10, int(img.width() * sx)))
    
    def commit_resize(self, event_x, event_y):
        """Commit resize transformation"""
        handle = self.drag_info['handle']
        x1, y1, x2, y2 = self.drag_info['start_bbox']
        ax = x2 if "left" in handle else x1
        ay = y2 if "top" in handle else y1
        ...
        for item in self.selected_items:
            img_name = self.get_image_name(item)
            if img_name:
                self.apply_transform(img_name, transform)
    ...
    def recolor_dialog(self):
        """Interactive recolor dialog"""
        if not self.selected_items:
            messagebox.showinfo("Recolor", "Please select items first")
            return
        ...
        if not colors:
            messagebox.showinfo("Recolor", "No colors found in selection")
            return
        
        color_to_replace = simpledialog.askstring(
            "Recolor", 
            f"Available colors:\n{', '.join(colors)}\n\nEnter color to replace:",
            parent=self.root
        )
        
        if color_to_replace and color_to_replace in colors:
            ...
            if new_color:
                ...
                self.update_status(f"Recolored {color_to_replace} to {new_color}")
